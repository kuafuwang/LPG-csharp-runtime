...
//* Console.Out.WriteLine("The offset adjustment is " + offset_adjustment);
            ...
            //
            LPG2.Runtime.ArrayList<IToken> affected_tokens = lexStream.getIPrsStream().incrementalResetAtCharacterOffset(start_change_offset); 
            ...
            if (affected_tokens.size() > 0) {
                if (affected_tokens.get(0).getEndOffset() + 1 < start_change_offset) {
                     ...
                     if (affected_tokens.get(0) is Token)
            ...
            int next_offset;
            do {
//* Console.Out.WriteLine("Scanning token starting at " + (lexStream.peek() - 1));            
                next_offset = lexParser.incrementalParseCharacters();
//* Console.Out.Write("***Remaining string: \"");
//*for (int i = next_offset; i < input_chars.Length; i++)
//* Console.Out.Write(input_chars[i]);
...
//*{
//* Console.Out.WriteLine("---Skipping token " + affected_index + ": \"" + affected_tokens.get(affected_index).toString() +
...
//*}
            ...
            for (int i = affected_index; i < affected_tokens.size(); i++) {
                if (affected_tokens.get(i) is Token)
...
//* Console.Out.WriteLine("+++Added affected token " + i + ": \"" + affected_tokens.get(i).toString() +
