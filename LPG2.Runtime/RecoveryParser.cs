...
public class RecoveryParser : DiagnoseParser 
{
   ...
   :     this(parser, monitor, action, tokens, tokStream, prs, 0, 0) {
    ...
    //
    public RecoveryParser(BacktrackingParser parser, Monitor monitor, IntSegmentedTuple action, IntTuple tokens, 
        IPrsStream tokStream, ParseTable prs, int maxErrors, long maxTime): base(monitor, tokStream, prs, maxErrors, maxTime)
    {
    ...
    protected override void reallocateStacks()
    {
        base.reallocateStacks();
        if (actionStack == null)
            actionStack = new int[stateStack.Length];
        else
        {
            int old_stack_length = actionStack.Length;
            Array.Copy(actionStack, 0, actionStack = new int[stateStack.Length], 0, old_stack_length);
        }
    ...
    public int recover(int marker_token, int error_token) 
    {
        if (stateStack == null)
            reallocateStacks();
...
//}
    ...
    // it returns.
    //
    private bool fixError(int start_token, int error_token)
    {
...
//                   " on symbol " + curtok + " ==> " + tokStream.getName(curtok));
    ...
    //
    //
    private bool completeScope(IntSegmentedTuple action, int scope_rhs_index)
    {
