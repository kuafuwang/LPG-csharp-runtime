...
public class DeterministicParser : Stacks
{
    ...
    //
    private  void processReductions()
    {
        do
        {
    ...
    public DeterministicParser(TokenStream tokStream, ParseTable prs, RuleAction ra)
    {
        reset(null, tokStream, prs, ra);
    }

    public DeterministicParser(Monitor monitor, TokenStream tokStream, ParseTable prs, RuleAction ra) 
    {
    ...
    public int parse(int[] sym, int index)
    {
            ...
            currentAction < ACCEPT_ACTION)  // SHIFT action ?
        {
            ...
            //
            // Process the const transition - either a shift action of
            // if we started out with a shift-reduce, the const GOTO
