...
//
    ...
    {
        private const  int TABLE_SIZE = 1021; // 1021 is a prime
        ...
        public bool findConfiguration(int[] stack, int stack_top, int curtok)
        {
            StateElement last_element = findOrInsertStack(state_root, stack, 0, stack_top);
            int hash_address = curtok % TABLE_SIZE;
            for (ConfigurationElement configuration = table[hash_address];
                configuration != null;
